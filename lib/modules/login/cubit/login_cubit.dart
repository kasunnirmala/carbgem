...
class LoginCubit extends Cubit<LoginState> {
  ...
  LoginCubit(this._authenticationRepository, this._bitteApiClient)
      : super(LoginState()) {
    getStorageData();
  }
  ...
  Future<void> cleanUpStorageData() async {
    if (state.rememberLogin) {
      ...
      await _storage.writeSecureData(
          key: 'password', value: state.password.value);
    } else {
  ...
  void emailChanged(String value) {
    ...
    emit(state.copyWith(
      email: email,
      status: LoginValidationState()
          .validate(status: Formz.validate([email, state.password])),
    ));
  }
  ...
  void passwordChanged(String value) {
    ...
    emit(state.copyWith(
      password: password,
      status: LoginValidationState()
          .validate(status: Formz.validate([state.email, password])),
    ));
  }
  ...
  Future<String> logIn() async {
    ...
    try {
      ...
      emit(state.copyWith(
          status: LoginStatus.submissionSuccess, errorMessage: ""));
      return state.errorMessage;
    } on Exception catch (e) {
      String errorMessage = getErrorMessage(exception: e);
      emit(state.copyWith(
          status: LoginStatus.submissionFailure, errorMessage: errorMessage));
      return state.errorMessage;
    }
  }
}
