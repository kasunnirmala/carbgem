...
part 'data_explorer_drug_state.dart';

class DataExplorerDrugCubit extends Cubit<DataExplorerDrugState> {
  final AuthenticationRepository _authenticationRepository;
  final BitteApiClient _bitteApiClient;

  DataExplorerDrugCubit(this._authenticationRepository, this._bitteApiClient): super(DataExplorerDrugState()){
    initiation();
  }
  Future<void> initiation() async{
    emit(state.copyWith(status: DataExplorerDrugStatus.loading));
    ...
    List<Antibiogram> dataList = await _bitteApiClient.getBreakdownDrugAPI(
    ...
    emit(state.copyWith(
      ...
      status: DataExplorerDrugStatus.success, dataList: dataList,
    ),);
  }
  Future<void> getTSList() async {
    ...
    emit(state.copyWith(dataList: dataList, status: DataExplorerDrugStatus.success));
  }
  Future<void> changeSpecimenType({required String specimenId}) async {
    ...
    emit(state.copyWith(
      ...
      status: DataExplorerDrugStatus.success,
    ),);
  }
  ...
  Future<void> changeArea({required AreaAPI selectArea}) async {
    ...
    emit(state.copyWith(dataList: dataList, selectArea: selectArea, status: DataExplorerDrugStatus.success));
  }
