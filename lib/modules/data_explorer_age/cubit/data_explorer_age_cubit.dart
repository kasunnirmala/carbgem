...
part 'data_explorer_age_state.dart';
class DataExplorerAgeCubit extends Cubit<DataExplorerAgeState> {
  final AuthenticationRepository _authenticationRepository;
  final BitteApiClient _bitteApiClient;

  DataExplorerAgeCubit(this._authenticationRepository, this._bitteApiClient): super(DataExplorerAgeState()){
    initiation();
  }
  Future<void> initiation() async{
    emit(state.copyWith(status: DataExplorerAgeStatus.loading));
    ...
    List<Antibiogram> dataList = await _bitteApiClient.getBreakdownAgeAPI(
    ...
    emit(state.copyWith(
      ...
      status: DataExplorerAgeStatus.success, dataList: dataList,
    ),);
  }
  Future<void> getTSList() async {
    ...
    emit(state.copyWith(dataList: dataList, status: DataExplorerAgeStatus.success));
  }
  Future<void> changeSpecimenType({required String specimenId}) async {
    ...
    emit(state.copyWith(
      ...
      status: DataExplorerAgeStatus.success,
    ),);
  }
  ...
  Future<void> changeArea({required AreaAPI selectArea}) async {
    ...
    emit(state.copyWith(dataList: dataList, selectArea: selectArea, status: DataExplorerAgeStatus.success));
  }
