...
class BasicHomeRoute extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => BasicHomeCubit(context.read<BitteApiClient>(),
          context.read<AuthenticationRepository>(), analytics, observer),
      child: BasicHomeView(
        analytics: analytics,
        observer: observer,
      ),
...
class _BasicHomeState extends State<BasicHome> {
  ...
  void initState() {
    ...
    final newVersion = NewVersion(
      iOSId: "com.carbgem.bitte.dev",
      androidId: "com.carbgem.bitte",
      ...
      child: BlocBuilder<BasicHomeCubit, BasicHomeState>(
        ...
        builder: (context, state) {
          return Scaffold(
            ...
            endDrawer: (state.status == BasicHomeStatus.activated)
                ? CustomDrawer()
                : CustomUnactivatedDrawer(),
            body: Container(
              child: Padding(
                    ...
                    child: (state.status == BasicHomeStatus.loading)
                        ...
                        : Column(
                            ...
                            children: [
                              ...
                              (state.status == BasicHomeStatus.unactivated)
                                  ? Container()
                                  : customTile(
                                      context: context,
                                      imagePath:
                                          'assets/images/patient_list.png',
                                      titleText:
                                          'Label_basic_home_topButton_title'
                                              .tr(),
                                      bodyTextList: [
                                        '${'Label_basic_home_topButton_body'.tr()}: ${state.currentUser.patientCount}'
                                      ],
                                      navIndex: 3,
                                    ),
                              ...
                              (state.status == BasicHomeStatus.unactivated)
                                  ...
                                  : customTile(
                                      context: context,
                                      imagePath: 'assets/images/virus.png',
                                      titleText:
                                          'Label_basic_home_middleButton_title'
                                              .tr(),
                                      bodyTextList: [
                                        '${"imageNum".tr()}: ${state.currentUser.unsortedCount}'
                                      ],
                                      navIndex: 5,
                                    ),
                              SizedBox(
                              ...
                              (state.status == BasicHomeStatus.unactivated)
                                  ...
                                  : customTile(
                                      context: context,
                                      imagePath:
                                          'assets/images/product_list_icon.png',
                                      titleText:
                                          "${"basic_home_userActivity_button_title".tr()}",
                                      bodyTextList: [
                                        "${"basic_home_userActivity_button_description".tr()}"
                                      ],
                                      navIndex: 9,
                                    ),
                              // SizedBox(height: 30,),
            ...
            bottomNavigationBar: (state.status == BasicHomeStatus.activated)
                ? CustomNavigationBar()
                : null,
          );
        },
...
class __ActivationButtonState extends State<_ActivationButton> {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<BasicHomeCubit, BasicHomeState>(
        builder: (context, state) => Container(
              ...
              child: RoundedLoadingButton(
                ...
                onPressed: () {
                  ...
                  context.read<AuthWizardCubit>().activationStep();
