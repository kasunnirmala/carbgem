  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        ...
        child: BlocProvider(
          create: (_) => ResendEmailCubit(context.read<AuthenticationRepository>()),
      ...
      listener: (context, state) {
        if (state==ResendEmailState.resendFailure){
          ScaffoldMessenger.of(context)
              ..hideCurrentSnackBar()
              ..showSnackBar(SnackBar(content: Text("Label_ResendEmail_resendFailure".tr()),));
        }
      },
      child: Center(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              SizedBox(height: 20,),
              Text("Label_ResendEmail_checkEmail".tr(), style: Theme.of(context).textTheme.headline6,),
...
class __ResendButtonState extends State<_ResendButton> {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<ResendEmailCubit, ResendEmailState>(builder: (context, state) {
